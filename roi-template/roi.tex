...
\\
TODO: VOIR POUR LES DEFINIR MATHEMATIQUEMENT\\
\textit{start, end}: DATE \comment first and last date of the file\\
\textit{$a\_start, a\_end$}: DATE \comment metadata evaluation period\\
\\
...
\where \\
	\tab \textit{m = di(a\_start)}\\ 
	\tab \textit{n = di(a\_end)}\\
\\
\\
